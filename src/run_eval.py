...
Sample usage:
export REF_FILE="data/codenet/splits/problem_id/2023-01-13_12-56pm/seq2seq_splits/test.jsonl" && 
export OP="/usr1/amadaan/learning2perf/data/outputs/beam_outputs_codegen_2b_jan_13_split.jsonl" && 
export CPU=30 && 
nohup python -u src/codenet_eval/run_eval.py  
    --model_generated_outputs_path ${OP} 
    --reference_file_path ${REF_FILE} 
    --output_report_file_path ${OP}.25_tries.report 
    --slow_code_col input # the column name in the jsonl
    --model_generated_potentially_faster_code_col beam_generated_target_from_input # [todo]:change the column name
    --reference_code_col target  # the column name in the jsonl
    --num_problems_to_evaluate -1  # evaluate all the programs when the val is -1
    --cpu_number $CPU 
    --max_time_per_run 10 
    --num_trials 25 
    --ignore_first_k 2
...
def evaluate_generated_outputs(cfg: EvaluationConfig) -> None: # todo: 1
    ...
    # Step 0
    merged = read_inputs_and_prepare(cfg) # todo: 2
    ...
    is_multigen = isinstance(merged[cfg.model_generated_potentially_faster_code_col].iloc[0], list) # todo: [cyu] pass@5 could happen here
    if is_multigen:
    ...
    results = run_programs(cfg, merged, problem_id_to_ground_truths, output_code_location, tag_to_path) # todo: 3
...
def read_inputs_and_prepare(cfg) -> pd.DataFrame:
    ...
    if cfg.reference_file_path is not None:
        ...
        print("warning: ", gen_df.shape, ref_df.shape)
        merged = pd.merge(
            gen_df,
            ref_df,
...
def run_programs(
    ...
    for _, row in tqdm(merged.iterrows(), total=len(merged), desc="running programs"):
        problem_id = row["problem_id"]
        submission_id_v0 = row["submission_id_v0"]
        unit_test_data_basepath = f"{cfg.inputs_outputs_basepath}/{problem_id}" # todo
        try:
            ...
            for (tag, suffix) in tag_to_path:
                ...
                avg_time, std_time, avg_acc = run_code_on_inputs(  # type: ignore # todo: 4
